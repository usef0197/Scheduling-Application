...
public class appointmentsMain {
    ...
    private FilteredList<Appointments> filteredAppointmentsList;
    ...
    public void initialize() throws SQLException {
        ...
        filteredAppointmentsList = new FilteredList<>(allAppointmentsList, p -> true);
        allAppointmentsTable.setItems(filteredAppointmentsList);

        // Bind the Predicate to the search field
        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredAppointmentsList.setPredicate(getAppointmentPredicate(newValue));
        });    }
    @FXML
    void searchButtonAction(ActionEvent event) {
        filteredAppointmentsList.setPredicate(getAppointmentPredicate(searchField.getText()));
    }

    private Predicate<? super Appointments> getAppointmentPredicate(String searchText) {
        return appointment -> {
            if (searchText == null || searchText.isEmpty()) {
                return true; // Show all appointments when search field is empty
            }

            String lowerCaseFilter = searchText.toLowerCase();

            // Customize this according to your search criteria
            return String.valueOf(appointment.getAppointmentID()).contains(lowerCaseFilter) ||
                    appointment.getAppointmentTitle().toLowerCase().contains(lowerCaseFilter) ||
