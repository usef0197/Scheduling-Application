...
public class addAppointments {
    ...
    @FXML
    public ComboBox<String> addAppointmentEndTime;
    ...
    @FXML
    public ComboBox<String> addAppointmentStartTime;
     ...
     */
    @FXML
    public void addAppointmentSave(ActionEvent event) throws IOException {
        try {
            ...
            if (!addAppointmentTitle.getText().isEmpty() && !addAppointmentDescription.getText().isEmpty() && !addAppointmentLocation.getText().isEmpty() && !addAppointmentType.getText().isEmpty() && addAppointmentStartDate.getValue() != null && addAppointmentEndDate.getValue() != null && !addAppointmentStartTime.getValue().isEmpty() && !addAppointmentEndTime.getValue().isEmpty() && !addAppointmentCustomerID.getText().isEmpty()) {
               ...
               LocalDateTime start = LocalDateTime.parse(appointmentStartDate+"T"+appointmentStartTime + ":00");
...
//                            (dateTimeEnd.isEqual(checkEnd) || dateTimeEnd.isBefore(checkEnd)))
                ...
                String insertStatement = "INSERT INTO appointments (Appointment_ID, Title, Description, Location, Type, Start, End, Create_Date, Created_By, Last_Update, Last_Updated_By, Customer_ID, User_ID, Contact_ID) VALUES (null,?,?,?,?,?,?,?,?,?,?,?,?,?)";

                DBConnection.setPreparedStatement(DBConnection.getConnection(), insertStatement);
                PreparedStatement ps = DBConnection.getPreparedStatement();
               // ps.setInt(1, newAppointmentID);
                ps.setString(1, addAppointmentTitle.getText());
                ps.setString(2, addAppointmentDescription.getText());
                ps.setString(3, addAppointmentLocation.getText());
                ps.setString(4, addAppointmentType.getText());
                ...
                ps.setInt(11, Integer.parseInt(addAppointmentCustomerID.getText()));
                ps.setInt(12, Integer.parseInt(contactAccess.findContactID(addAppointmentUserID.getText())));
